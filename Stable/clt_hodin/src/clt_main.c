...
\tcomputer across a computer network.\"\n\n\t\t\t\t\t\t\t\t[ Wikipedia ]", -1);
    ...
    /* TAB CLIENTS VARS*/
    GtkWidget *client_number = NULL;
    GtkWidget *clients_number_label = NULL;
    
     GtkWidget *apply_button = NULL;
    
    clients_number_label = gtk_label_new("Enter numbers of clients : ");
    gtk_fixed_put(GTK_FIXED(zone[3]), clients_number_label, 20, 20);
    
    client_number = gtk_spin_button_new_with_range(1, 10, 1);
    gtk_widget_set_size_request(client_number, 100, 30);
    gtk_fixed_put(GTK_FIXED(zone[3]), client_number, 180, 10);
    
    apply_button = gtk_button_new_with_label("Apply");
    gtk_widget_set_size_request(apply_button, 100, 30);
    gtk_fixed_put(GTK_FIXED(zone[3]), apply_button, 300, 10);
    //g_signal_connect(G_OBJEC(apply_button), "clicked", G_CALLBACK(cb_generate_client), NULL);
