...
sudo dnf install https://download1.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://download1.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm -qq -y 2>/dev/null

sudo dnf install ffmpeg ffmpeg-devel -qq -y 2>/dev/null
...
sudo dnf install gstreamer1.0-alsa -qq -y 2>/dev/null
